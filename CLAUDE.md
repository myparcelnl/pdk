...
- Snapshots generated by Pest should be formatted with Prettier to ensure consistent formatting across the codebase. Use the `test:unit:snapshot` script defined in `package.json` to update snapshots and format them with Prettier in one step: `yarn test:unit:snapshot`
