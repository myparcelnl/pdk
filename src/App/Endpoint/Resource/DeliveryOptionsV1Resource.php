...
class DeliveryOptionsV1Resource extends AbstractVersionedResource
{
    ...
    public static function format($data): array
    {
        ...
        return [
            'orderId' => $orderId,
            'deliveryOptions' => self::formatDeliveryOptions($deliveryOptions),
        ];
    }
    ...
    private static function formatDeliveryOptions(?DeliveryOptions $deliveryOptions): array
    {
        ...
        return [
            'carrier' => self::formatCarrier($deliveryOptions->carrier->name),
            'packageType' => self::formatPackageType($deliveryOptions->packageType),
            'deliveryType' => self::formatDeliveryType($deliveryOptions->deliveryType),
            'shipmentOptions' => self::formatShipmentOptions($deliveryOptions),
            // format date as ISO 8601 string or null
            'date' => $deliveryOptions->date ? $deliveryOptions->date->format('c'): null,
    ...
    private static function formatShipmentOptions(DeliveryOptions $deliveryOptions): array
    {
        ...
        foreach (ShipmentOptions::ALL_SHIPMENT_OPTIONS as $shipmentOptionName) {
            if ($shipmentOptions->{$shipmentOptionName}) {
                $constantName = Str::upper(Str::snake($shipmentOptionName));
