...
return [
    ...
    'builtInDefaultSettings' => factory(fn() => [
        CarrierSettings::ID => [
            SettingsManager::KEY_ALL => [
                ...
                CarrierSettings::DROP_OFF_POSSIBILITIES                  => [
                    'dropOffDays'           => array_map(static fn(int $weekday) => [
                        'cutoffTime'        => '17:00:00',
                        'dispatch'          => true,
    ...
    'mergedDefaultSettings' => factory(fn() => array_replace_recursive(
    ...
    'settingKeyPrefix' => factory(fn() => sprintf('%s_', PdkFacade::getAppInfo()->name)),

    /**
     * Callback to generate a settings key.
     */

    'createSettingsKey' => factory(
        fn() => static fn(string $key) => sprintf('%s%s', PdkFacade::get('settingKeyPrefix'), $key)
    ),
    ...
    'createBarcodeNoteIdentifier' => factory(fn() => static fn(string $shipmentId) => "barcode-$shipmentId"),

    /**
     * Settings key where the installed version of the plugin is saved.
     */

    'settingKeyVersion' => factory(fn() => PdkFacade::get('createSettingsKey')('version')),

    /**
     * Settings key where webhooks are saved
     */

    'settingKeyWebhooks' => factory(fn() => PdkFacade::get('createSettingsKey')('webhooks')),

    /**
     * Settings key where the full webhook url is saved
     */

    'settingKeyWebhookHash' => factory(fn() => PdkFacade::get('createSettingsKey')('webhook_hash')),

    /**
     * Settings key where the installed version of the app is saved.
     */

    'settingKeyInstalledVersion' => factory(fn() => PdkFacade::get('createSettingsKey')('installed_version')),
];
